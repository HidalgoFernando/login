    ...
    if ($nr == 0) {
        ...
        if ($conn->query($insertarRegistro) === TRUE) {
            ...
            if ($conn->query($insertarDetalles) === TRUE) {
                echo "<script>alert('Se registró con éxito'); window.location = '..//frontend//registro.html';</script>";
            } else {
        ...
        echo "<script>alert('No puedes registrar este correo: $correo'); window.location = '..//frontend//registro.html';</script>";
    }
} else {
